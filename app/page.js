  ...
  return(
    <div style={{background:isEchi?DARK2:CARD,borderRadius:12,overflow:"hidden",border:`1px solid ${isEchi?"#1E2E40":BD}`,transition:"transform 0.2s,box-shadow 0.2s",cursor:"pointer"}}
      ...
      <div style={{padding:featured?"24px 24px 20px":"16px 18px 14px"}}>
        ...
        {article.audio_url&&<div style={{marginBottom:14,padding:"10px 12px",background:isEchi?"#0D151F":LIGHT,borderRadius:8}} onClick={e=>e.stopPropagation()}><div style={{fontSize:9,fontFamily:"'Orbitron'",letterSpacing:1.5,color:TD,marginBottom:6,textTransform:"uppercase"}}>🎵 Ascolta</div><audio controls src={article.audio_url} style={{width:"100%",height:36}} preload="none"/></div>}
  ...
  return(
    <div style={{maxWidth:1200,margin:"0 auto",padding:"32px 20px 48px",animation:"fadeUp 0.4s ease",display:"flex",gap:32}}>
      <div style={{flex:1,minWidth:0}}>
        ...
        {article.audio_url&&<div style={{marginBottom:32,padding:"16px 20px",background:LIGHT,borderRadius:12,border:`1px solid ${BD}`}}><div style={{fontSize:10,fontFamily:"'Orbitron'",letterSpacing:1.5,color:AC,marginBottom:10,textTransform:"uppercase",fontWeight:600}}>🎵 Ascolta la traccia</div><audio controls src={article.audio_url} style={{width:"100%"}} preload="none"/></div>}
